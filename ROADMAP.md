...
- [ ] **W1c-25-5 — `wlr-screencopy-v1` thumbnail handoff** — **DEFERRED**. Smithay 0.7 does not ship a server-side `wlr-screencopy-v1` implementation. Implementing the protocol from raw `wayland-protocols-wlr` bindings is multi-session work (frame copy semantics, dmabuf handling, per-frame negotiation, then a second session for GTK4 buffer import on the overlay side). Three tractable alternatives to evaluate when there's a visual budget for the work: (a) wait for smithay to add screencopy support upstream; (b) custom IPC for periodic RGB buffer push (vibewm → daemon → overlay) tailored to overlay's per-cluster thumbnail need — same effective shape, smaller scope than a full protocol; (c) shrink scope to per-output snapshots only. Without thumbnails the W1c-25-1/3/4 smooth-zoom is mechanically smooth but visually unconvincing — empty cluster cards growing to fullscreen.
