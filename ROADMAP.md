...
- [ ] **W1c-16 — DRM backend, first pass** (2026-04-29, in flight): `apps/vibewm/src/udev.rs` ships a single-GPU, single-output, no-hot-plug DRM compositor scaffolded against smithay 0.7's `LibSeatSession` + `UdevBackend` + `LibinputInputBackend` + `DrmDevice` + `DrmCompositor` + GBM/EGL/GLES. New `udev` Cargo feature (`smithay/backend_libinput,backend_udev,backend_drm,backend_gbm,backend_session_libseat`); selected at runtime by `VIBEWM_BACKEND=udev` (default `winit`). `Vibewm` gained an `Option<UdevState>` field. **First-pass status**: structural port from anvil, won't fully build until tested in a VM with libseat-devel installed; expect API-drift fixes between smithay 0.7 and anvil's master in the first iteration. Won't render anything visible yet (no cursor, no live verification). Still-stubbed: hot-plug, multi-output, multi-GPU, DRM lease, dmabuf protocol, drm-syncobj, smithay-drm-extras EDID, cursor plane, damage-tracked partial redraw.
- [ ] **W1c-17+** — DRM iterate-to-green: fix API mismatches surfaced by VM build, get a frame on screen, draw a cursor; smooth zoom transitions; live thumbnails; libinput gesture integration.
