...
- [ ] **W1c-6** — `MoveFocusedWindowToCluster` IPC + CLI; restore panel right-click. Layer-surface map/destroy logging in vibewm.
- [ ] **W1c-7+** — xdg-shell move/resize grabs wired via the daemon drag flow; smooth zoom transitions; live thumbnails (smithay scene graph); DRM backend (replace winit); libinput gesture integration; xwayland.
